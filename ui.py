...
class ChipInSightUI:
    ...
    def _init_match_dialog(self):
        # 先创建 dialog，再创建内部 table
        self.buy_select_dialog = ui.dialog()
        with self.buy_select_dialog, ui.card().classes('w-[700px] p-4'):
            ...
            dialog = self.buy_select_dialog
            ui.button("关闭", on_click=lambda: dialog.close()).classes('mt-3').props('outline')

    async def _open_match_dialog(self, e):
        if not self.buy_select_dialog:
            return
        row = e.args[1]
    ...
    async def _load_available_buys(self):
        if not self.current_selected_stock or not self.available_buy_table:
            return
        try:
    ...
    async def refresh_sell_match_table(self):
        if not self.current_selected_stock or not self.sell_match_table:
            return
        try:
    ...
    async def refresh_chip_price(self):
        if not self.current_selected_stock or not self.chip_price_table:
            return
        try:
    ...
    async def refresh_chip_summary(self):
        if not self.chip_summary_table:
            return
        try:
